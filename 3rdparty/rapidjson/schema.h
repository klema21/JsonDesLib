...
private:
    ...
    // TODO cache pointer <-> id mapping
    ValueType* FindId(const ValueType& doc, const UriType& finduri, PointerType& resptr, const UriType& baseuri, bool m_full, const PointerType& here = PointerType()) const {
        SizeType i = 0;
        ValueType* resval = 0;
        ...
        if (doc.GetType() == kObjectType) {
            ...
            // See if it matches
            if (localuri.Match(finduri, m_full)) {
            ...
            for (m = doc.MemberBegin(); m != doc.MemberEnd(); ++m) {
                if (m->value.GetType() == kObjectType || m->value.GetType() == kArrayType) {
                    resval = FindId(m->value, finduri, resptr, localuri, m_full, here.Append(m->name.GetString(), m->name.GetStringLength(), allocator_));
                }
                if (resval) break;
            }
            ...
            for (typename ValueType::ConstValueIterator v = doc.Begin(); v != doc.End(); ++v) {
                if (v->GetType() == kObjectType || v->GetType() == kArrayType) {
                    resval = FindId(*v, finduri, resptr, localuri, m_full, here.Append(i, allocator_));
                }
                if (resval) break;
                i++;
            }
